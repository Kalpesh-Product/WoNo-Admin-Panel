...
const Calender = () => {
  ...
  const { data: meetings = [], isPending: isEventsPending } = useQuery({
    queryKey: ["meetings"],
    queryFn: async () => {
      try {
  ...
  useEffect(() => {
    if (eventFilter.length === 0) {
      setFilteredEvents(meetings);
    } else {
      const filtered = meetings.filter((event) =>
          ...
          {drawerMode === "view" && selectedEvent && (
            <div>
              <div className="flex flex-col gap-2">
                ...
                <DetalisFormatted
                  title={"Time"}
                  detail={`${dayjs(selectedEvent.start).format("hh:mm A")} - ${dayjs(selectedEvent.end).format("hh:mm A")}`}
                />
                {selectedEvent.extendedProps?.agenda && (
                  <div className="space-y-2">
                    ...
                    />
                    <DetalisFormatted title={"Location"} detail={`${selectedEvent.extendedProps.location?.unitNo} ${selectedEvent.extendedProps.roomName}`} />
                  </div>
                )}
              </div>
            </div>
